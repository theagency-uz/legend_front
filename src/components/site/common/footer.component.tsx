...
export default async function Footer({ lang }: { lang: string }) {
  ...
  return (
    <footer className="px-[100px] py-[40px] pb-[61px] max-xs:px-[10px] bg-white max-sm:px-[10px] max-md:px-[20px]">
          ...
          <ul className="flex flex-col gap-[5px]">
            {FOOTER_CATALOG_LINKS.map(({ text, href }, index) => {
              return (
                <li key={index}>
                  <Link
                    className="small-normal-nospacing text-primary-100"
                    href={href}
                  >
                    {t(text)}
                  </Link>
      ...
      <div className="flex justify-between max-md:flex-col max-md:gap-[30px]">
        <div className="flex flex-col gap-[22px] max-xs:gap-[10px] text-primary-100 small-normal-nospacing">
        ...
        <Link
          href=""
          className="underline text-primary-100 small-normal-nospacing self-end max-md:self-start"
        >
          {t("Разработка сайта")}
        </Link>
      </div>
